...
**Mermaid 图规范**:
1. 使用 `graph LR`（左到右）布局，数据流图天然从左到右更直观
2. 节点换行**必须用 `<br/>`，禁止用 `\n`**（`\n` 会原样显示为文字）
3. **训练和推理拆成两个独立 Mermaid 代码块**，不要合并在一个图里；合并会导致跨阶段 back-edge 破坏布局
4. subgraph 按**数据角色**分组：`观测`、`网络`、`输出/反馈`；训练图额外加 `训练设置` 放课程/超参/仿真环境
5. **subgraph 标签不能为空字符串** `[""]`，会 parse error；必须给名字
6. **多个输入 concat 到同一节点时，每个输入都单独画箭头到该节点**，不要 A→B→C（会误读为 B 是中间处理步骤）
7. Mermaid **无法精确控制 2D 布局**，节点位置由拓扑自动决定；不要试图用嵌套 subgraph 强制"顶部/左侧"，会让图更乱
8. **⚠️ 所有包含特殊字符的节点必须用 `["..."]` 包裹**：Unicode 数学符号（∇², ∂Ω, ℝ, ∈, ∀, ⊂, ⇒, φ, ⊖, ‖, ḣ, α 等）在 `[...]` 中会被 Mermaid 解析异常，必须改为 `["..."]`。安全规则：**拿不准就加引号，不加反而不安全。**
9. **⚠️ 避免节点文本中出现嵌套方括号**：Mermaid 会将 `[` 解析为新节点标识。例如 `[0,T]` 应改为 `(0,T)`，`{1,...,N}` 保持原样（花括号安全）。使用 `[...]` 或 `(...)` 替代所有内层方括号。
10. **推理图只画 actor 实际用到的输入**；训练时 Critic 的特权观测（GT 力、速度等）在部署时不存在，不要画进推理图
...
**自检清单**:
- [ ] 训练和推理是**两个独立的 mermaid 代码块**？
- [ ] 节点换行全部用 `<br/>`，没有 `\n`？
- [ ] subgraph 按数据角色分组（观测 / 网络 / 输出）？
- [ ] subgraph 标签非空？
- [ ] concat 输入每个都有独立箭头到目标节点？
- [ ] 推理图只含 actor 实际接收的输入（无训练特权观测）？
- [ ] 节点数量合理（8-20 个）？
- [ ] **论文原图已引用**在 Mermaid 图上方（见上方 📌 论文架构图引用）？
- [ ] **所有特殊字符节点用 `["..."]` 包裹**？（如 ∇²、∂Ω、ℝ、∈、∀、φ 等 → `["节点文本"]`）
- [ ] **无嵌套方括号**？（如 `[0,T]` → `(0,T)`）
