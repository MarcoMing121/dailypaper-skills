...
6. 关键模块用 `[[概念]]` 标注（如果需要）
7. **⚠️ 所有包含特殊字符的节点必须用 `["..."]` 包裹**：Unicode 数学符号（∇², ∂Ω, ℝ, ∈, ∀, ⊂, ⇒, φ, ⊖, ‖, ḣ, α 等）在 `[...]` 中会被 Mermaid 解析异常，必须改为 `["..."]`。安全规则：**拿不准就加引号，不加反而不安全。**
8. **⚠️ 避免节点文本中出现嵌套方括号**：Mermaid 会将 `[` 解析为新节点标识。例如 `[0,T]` 应改为 `(0,T)`，`{1,...,N}` 保持原样（花括号安全）。使用 `[...]` 或 `(...)` 替代所有内层方括号。
